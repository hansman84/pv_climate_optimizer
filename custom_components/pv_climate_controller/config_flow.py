...
def _schema(defaults: dict[str, Any] | None = None) -> vol.Schema:
    ...
    schema[vol.Required(CONF_MIN_PV_SURPLUS_W, default=values.get(CONF_MIN_PV_SURPLUS_W, 150.0))] = vol.All(vol.Coerce(float), vol.Range(min=50, max=20000))
...
class PVClimateControllerOptionsFlow(config_entries.OptionsFlow):
    ...
    async def async_step_thermal(self, user_input: dict[str, Any] | None = None) -> FlowResult:
        ...
        return self.async_show_form(step_id="thermal", data_schema=vol.Schema({
            ...
            vol.Optional(CONF_WEATHER_FORECAST_ENTITY_ID, default=values.get(CONF_WEATHER_FORECAST_ENTITY_ID)): EntitySelector(EntitySelectorConfig(domain="weather", multiple=False)),
            vol.Optional(CONF_OUTDOOR_RELAXATION_BAND_C, default=values.get(CONF_OUTDOOR_RELAXATION_BAND_C, 1.5)): vol.All(vol.Coerce(float), vol.Range(min=0.5, max=5.0)),
            vol.Optional(CONF_OUTDOOR_NO_ACTIVE_COOLING_C, default=values.get(CONF_OUTDOOR_NO_ACTIVE_COOLING_C, 0.5)): vol.All(vol.Coerce(float), vol.Range(min=0.0, max=5.0)),
            vol.Optional(CONF_OUTDOOR_RAIN_HOLD_PROBABILITY_PCT, default=values.get(CONF_OUTDOOR_RAIN_HOLD_PROBABILITY_PCT, 60.0)): vol.All(vol.Coerce(float), vol.Range(min=0.0, max=100.0)),
            vol.Optional(CONF_OUTDOOR_PV_BOOST_EXTRA_W, default=values.get(CONF_OUTDOOR_PV_BOOST_EXTRA_W, 2000.0)): vol.All(vol.Coerce(float), vol.Range(min=0.0, max=20000.0)),
        }))
...
def _energy_schema(defaults: dict[str, Any] | None = None) -> vol.Schema:
    ...
    schema: dict[Any, Any] = {
        ...
        vol.Required(CONF_MIN_PV_SURPLUS_W, default=values.get(CONF_MIN_PV_SURPLUS_W, 150.0)): vol.All(vol.Coerce(float), vol.Range(min=50, max=20000)),
