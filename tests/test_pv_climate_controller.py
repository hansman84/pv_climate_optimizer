...
def test_child_bedroom_uses_its_own_pre_cooling_start_time() -> None:
    bedroom = models.ZoneConfig("bedroom", "Schlafzimmer", "climate.bedroom", "sensor.bedroom")
    ...
    runtime = controller.PVClimateController(
        config=models.ControllerConfig(
            ...
            living_room_pilot_enabled=True, house_zones=(bedroom, child),
            bedroom_mode_enabled=True, bedroom_start_time="15:00",
            child_bedroom_start_time="17:00", bedroom_cutoff_enabled=False,
        ),
        command_adapter=adapter.ClimateCommandAdapter(shadow_mode=False, productive_enabled=True),
    )
    runtime.last_energy = models.EnergySnapshot(export_power_w=500)

    master = runtime.decide_bedroom_pilot(bedroom, temperature_c=24.0, climate_mode="off", now=time(16, 0))
    child_action = runtime.decide_bedroom_pilot(child, temperature_c=24.0, climate_mode="off", now=time(16, 0))

    assert master.reason_code != "bedroom_window_pending"
    assert child_action.reason_code == "bedroom_window_pending"
...
def test_existing_schlafzimmer_mapping_is_disabled_until_explicitly_enabled() -> None:
    bedroom = controller._house_zones([{
        ...
        "climate_entity_id": "climate.bedroom", "temperature_entity_id": "sensor.bedroom",
    }])[0]
    runtime = controller.PVClimateController(
        config=models.ControllerConfig(
            shadow_mode=False, energy_policy=const.EnergyPolicy.PV_PREFERRED,
    ...
    action = runtime.decide_bedroom_pilot(bedroom, temperature_c=24.0, climate_mode="off", now=time(16, 0))

    assert not bedroom.pilot_enabled
    assert action.reason_code == "zone_pilot_disabled"
...
def test_pilot_raises_target_then_stops_only_if_thermal_relief_fails() -> None:
    ...
    runtime = models.ControllerConfig(
        shadow_mode=False,
        energy_policy=const.EnergyPolicy.PV_PREFERRED,
        zone=models.ZoneConfig("office", "Arbeitszimmer", "climate.office", "sensor.office", comfort_temperature=23.5),
        living_room_pilot_enabled=True,
        min_pv_surplus_w=100,
    )
    room_pilot = pilot.LivingRoomPilot(clock, expected_zone_name="Arbeitszimmer", display_name="Arbeitszimmer")
    room_pilot.mark_sent(pilot.PilotAction("start", 24.0, "test", "test"))

    confirming = room_pilot.decide(
        runtime,
        temperature_c=22.6,
        climate_mode="cool",
        granted_stages=1,
        ...
        predicted_temperature_60m_c=21.94,
    )
...
def test_living_room_pilot_relaxes_before_stopping_when_export_reaches_zero() -> None:
    clock = Clock()
    runtime = models.ControllerConfig(
        shadow_mode=False,
        energy_policy=const.EnergyPolicy.STRICT_PV,
    ...
    assert (wind_down.action, wind_down.target_temperature_c, wind_down.reason_code) == ("adjust", 24.0, "pv_comfort_hold")
    living_pilot.mark_sent(wind_down)


def test_living_room_pilot_only_relaxes_after_authoritative_room_temperature_reaches_comfort_band() -> None:
    """A PV dip must not raise a warm living room directly to 25 °C."""
    clock = Clock()
    runtime = models.ControllerConfig(
        shadow_mode=False,
        energy_policy=const.EnergyPolicy.STRICT_PV,
        zone=models.ZoneConfig("living", "Wohnzimmer", "climate.living", "sensor.living", comfort_temperature=23.5),
        living_room_pilot_enabled=True,
        min_pv_surplus_w=1000,
    )
    living_pilot = pilot.LivingRoomPilot(clock)
    living_pilot.mark_sent(pilot.PilotAction("start", 20.0, "test", "test"))

    warm_room = living_pilot.decide(runtime, temperature_c=24.53, climate_mode="cool", granted_stages=1, export_power_w=0)
    assert (warm_room.action, warm_room.target_temperature_c, warm_room.reason_code) == ("adjust", 24.0, "pv_comfort_hold")
    living_pilot.mark_sent(warm_room)

    in_comfort_band = living_pilot.decide(runtime, temperature_c=23.7, climate_mode="cool", granted_stages=1, export_power_w=0)
...
def test_heat_pump_priority_raises_only_one_degree_per_minute_step() -> None:
    ...
    living_pilot = pilot.LivingRoomPilot(clock)
    living_pilot.mark_sent(pilot.PilotAction("start", 21.0, "test", "test"))
    clock.now = 1800

    relief = living_pilot.decide(
...
def test_warm_living_room_keeps_comfort_hold_even_at_low_power_without_pv() -> None:
    clock = Clock()
    runtime = models.ControllerConfig(
        shadow_mode=False,
        energy_policy=const.EnergyPolicy.STRICT_PV,
    ...
    assert (action.action, action.reason_code, action.planned_target_temperature_c) == ("adjust", "pv_comfort_hold", 24.0)
...
def test_every_productive_room_pilot_relaxes_to_its_own_upper_target_before_stopping() -> None:
    ...
    for zone_name in ("Spielzimmer", "Speis", "Schlafzimmer", "Kinderzimmer"):
        ...
        runtime = models.ControllerConfig(
            ...
            zone=models.ZoneConfig(
                zone_name.casefold(), zone_name, f"climate.{zone_name.casefold()}", f"sensor.{zone_name.casefold()}",
                pilot_min_target_temperature=22.0,
        ...
        room_pilot = pilot.LivingRoomPilot(clock, expected_zone_name=zone_name, display_name=zone_name)
        room_pilot.mark_sent(pilot.PilotAction("start", 23.0, "test", "test"))
        clock.now = room_pilot._PV_WIND_DOWN_FAST_STEP_S

        wind_down = room_pilot.decide(
...
def test_wind_down_ramps_slower_when_some_pv_remains() -> None:
    ...
    waiting = living_pilot.decide(runtime, temperature_c=24.5, climate_mode="cool", granted_stages=1, export_power_w=500)
    assert waiting.reason_code == "pv_comfort_holding"

    clock.now += living_pilot._PV_WIND_DOWN_FAST_STEP_S
    second = living_pilot.decide(runtime, temperature_c=24.5, climate_mode="cool", granted_stages=1, export_power_w=500)
    assert (second.action, second.planned_target_temperature_c) == ("none", 24.0)
...
def test_living_room_pilot_reclaims_manual_cooling_at_pv_evening_deadline() -> None:
    ...
    assert (at_deadline.action, at_deadline.target_temperature_c, at_deadline.reason_code) == ("adjust", 24.0, "pv_comfort_hold")
...
def test_living_room_pilot_ignores_unrelated_fan_attribute_churn() -> None:
    ...
    action = living_pilot.decide(
        runtime, temperature_c=24.2, climate_mode="cool", granted_stages=1, export_power_w=1200,
        climate_target_temperature_c=23.0, climate_fan_mode="auto", climate_swing_mode="auto",
    )
